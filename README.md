...
| **深度思考模式** | 多智能体多轮预演：教师讲稿 → 第一轮学生反应 → 第二轮讨论/反方挑战/裁决 → 教研汇总 | 需要更真实的课堂讨论感、希望获得更稳健建议时 |

深度思考模式的大致流程：

1. **教师智能体**：读取教案/逐字稿，生成完整课堂讲稿，并拆成 3～5 个教学模块（随材料长度自动调整）。
2. **第一轮学生智能体**：按当前学科配置的学生画像（可自定义人数与层级），在每个模块听完讲解后先给出原始课堂反应。
...
- 模拟对象：基础薄弱型、中等偏下型、中等稳定型、中等偏上型、拔高拓展型
- 学生画像按学科自动切换：文科偏阅读理解与证据表达，理科偏步骤推理与条件检验
- 提供画像编辑面板：可按学科自定义五类学生画像并保存
- 支持画像模板 JSON 导入/导出：便于教研组共享与复用
- 支持多种模型 API：DeepSeek、Qwen、GLM、OpenAI、Gemini、Claude、Kimi、MiniMax
- 输出内容：
  - 关键知识点提取
  - 五类学生反应与可能错误预测（快速模式一次生成；深度模式基于模块内多轮讨论后汇总）
  - 课程难度评估
  - 教学优化建议
  - AI 修改教案的摘要说明
  - 原教案 / 修订后教案对照查看
...
- 或直接粘贴文本
- 选择学科与年级
- 如需定制课堂画像，可在侧边栏“编辑当前学科学生画像”中修改并保存
- 画像编辑面板中的 5 个槽位层级固定，依次对应 low / mid-low / mid / mid-high / high
- 其中“需要的教学支持”指教师可额外提供的讲解支架、提示方式或练习安排
- “优势”表示这类学生通常做得比较好的地方；“薄弱点”表示最容易卡住的地方；“常见错误”表示课堂和作业中最常出现的错法
- 可导出当前学科画像为 JSON，或导入 JSON 覆盖当前学科模板
...
## Windows 打包与分发（给老师双击运行）

如果你希望老师在其他电脑上直接双击运行，不需要手动输入命令，可以使用项目内置打包脚本。
...
- 安装 PyInstaller
- 生成可分发目录：`dist/TeachingAI`
- 产出可执行文件：`dist/TeachingAI/TeachingAI.exe`

### 2. 分发给老师

把整个文件夹 `dist/TeachingAI` 拷贝到目标 Windows 电脑。
...
- 目标电脑需要联网（因为模型 API 调用在云端）。
- 首次启动可能稍慢（OCR/Streamlit 组件初始化）。
- 若启用 OCR，建议目标机有足够内存和 CPU 资源。
- 通过 `TeachingAI.exe` 启动时，程序会监听页面心跳；关闭老师打开的浏览器页面后一小段时间内，后台进程会自动退出。
- 不要只拷贝 `exe` 单文件，必须拷贝整个 `dist/TeachingAI` 目录。
