...
def _difficulty_color(label: str) -> str:
    return {"易": "#047857", "中": "#B45309", "难": "#B91C1C"}.get(label, "#374151")


def _accuracy_bar_html(accuracy: float, difficulty_label: str) -> str:
    pct = int(accuracy * 100)
    color = {"易": "#10B981", "中": "#F59E0B", "难": "#EF4444"}.get(difficulty_label, "#6B7280")
    return (
        f"<div style='background:#F3F4F6;border-radius:6px;height:8px;width:100%;margin:4px 0;'>"
        f"<div style='background:{color};width:{pct}%;height:8px;border-radius:6px;'></div>"
        f"</div>"
        f"<span style='font-size:11px;color:#6B7280;'>{pct}% 预测正确率</span>"
    )


def render_exam_results(report: ExamReport) -> None:
    st.success(f"试卷分析完成（{report.subject} · {report.grade} · {report.exam_topic}）")

    # ---- Overview stats ----
    total_points = sum(q.points for q in report.questions)
    ...
    st.markdown(
        f"""
        <div style='border:1px solid #E6E2D8;border-radius:14px;padding:14px 18px;
                    background:linear-gradient(180deg,#FCFBF7 0%,#F7F4EB 100%);margin-bottom:14px;'>
            <div style='font-size:21px;font-weight:700;color:#1E3A2E;margin-bottom:6px;'>试卷分析总览</div>
            <div style='font-size:13px;color:#5C5A53;'>基于学生画像模拟各题作答情况，识别教学薄弱点并给出改进建议。</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    ov_c1.metric("题目总数", len(report.questions))
    ov_c2.metric("总分", f"{total_points:.0f} 分")
    ov_c3.metric("模拟学生数", len({a.profile_name for a in report.student_attempts}))
    low_count = sum(1 for d in report.question_diagnostics if d.predicted_class_accuracy < 0.5)
    ov_c4.metric("预测失分题数", low_count, delta=None, help="预测全班正确率低于50%的题目数量")

    if type_desc:
        st.caption(f"题型分布：{type_desc}")

    # ---- Class summary ----
    if report.class_summary:
        st.info(report.class_summary)
    ...
    diag_by_qid = {d.question_id: d for d in report.question_diagnostics}

    for q in report.questions:
        diag = diag_by_qid.get(q.question_id)
        diff_label = diag.difficulty_label if diag else "中"
        acc = diag.predicted_class_accuracy if diag else 0.5
        diff_color = _difficulty_color(diff_label)

        header_html = (
            f"<span style='color:{diff_color};font-weight:700;'>[{diff_label}]</span> "
            f"{q.question_id.upper()} · {q.question_type} · {q.points:.0f}分  "
            f"<span style='font-size:12px;color:#6B7280;'>预测正确率 {int(acc*100)}%</span>"
        )
        with st.expander(f"{q.question_id.upper()} · {q.question_type} · {q.points:.0f}分 — {diff_label} — {int(acc*100)}%正确率", expanded=False):
            st.markdown(header_html, unsafe_allow_html=True)
            st.markdown(_accuracy_bar_html(acc, diff_label), unsafe_allow_html=True)

            # Knowledge tags
            if q.knowledge_tags:
                tags_html = "".join(
                    f"<span style='display:inline-block;font-size:11px;border-radius:999px;"
                    f"padding:2px 8px;background:#EEF4ED;color:#2A4F3C;"
                    f"border:1px solid #CFDCCC;margin-right:5px;'>{t}</span>"
            ...
            if diag:
                c1, c2 = st.columns(2)
                with c1:
                    if diag.common_error_type:
                        st.markdown(f"**常见错误类型**：{diag.common_error_type}")
                with c2:
                    if diag.teaching_note:
                        st.markdown(f"**教学诊断**：{diag.teaching_note}")

            # Per-student attempts for this question
            q_attempts = [a for a in report.student_attempts if a.question_id == q.question_id]
            if q_attempts:
                st.markdown("**各学生预测表现**")
                attempt_cols = st.columns(len(q_attempts))
                for col, attempt in zip(attempt_cols, q_attempts):
                    score_ratio = attempt.estimated_score / q.points if q.points > 0 else 0
                    card_color = "#DCFCE7" if score_ratio >= 0.8 else ("#FEF9C3" if score_ratio >= 0.4 else "#FEE2E2")
                    text_color = "#166534" if score_ratio >= 0.8 else ("#92400E" if score_ratio >= 0.4 else "#991B1B")
                    with col:
                        st.markdown(
                            f"<div style='border-radius:8px;padding:8px 10px;background:{card_color};"
                            f"margin-bottom:6px;'>"
                            f"<div style='font-weight:700;font-size:13px;color:{text_color};'>"
                            f"{attempt.profile_name}</div>"
                            f"<div style='font-size:13px;color:{text_color};'>"
                            f"{attempt.estimated_score:.0f}/{q.points:.0f}分</div>"
                            f"<div style='font-size:11px;color:#374151;margin-top:4px;'>"
                            f"{attempt.error_type}</div>"
                            f"</div>",
                            unsafe_allow_html=True,
                        )
                        if attempt.reasoning_note:
                            st.caption(attempt.reasoning_note)

    st.markdown("---")

    # ---- Per-student performance matrix ----
    st.subheader("学生作答预测汇总")

    profile_names = sorted({a.profile_name for a in report.student_attempts})
    if profile_names:
        student_tabs = st.tabs(profile_names)
        for tab, name in zip(student_tabs, profile_names):
            with tab:
                name_attempts = {a.question_id: a for a in report.student_attempts if a.profile_name == name}
                total_estimated = sum(a.estimated_score for a in name_attempts.values())
                correct_count = sum(1 for a in name_attempts.values() if a.predicted_correct)
                st.markdown(
                    f"**预测总分**: {total_estimated:.0f} / {total_points:.0f} 分 &nbsp;|&nbsp; "
                    f"**预测答对题数**: {correct_count} / {len(report.questions)}"
                )

                for q in report.questions:
                    attempt = name_attempts.get(q.question_id)
                    if attempt is None:
                        continue
                    score_ratio = attempt.estimated_score / q.points if q.points > 0 else 0
                    icon = "✅" if attempt.predicted_correct else ("⚠️" if score_ratio >= 0.4 else "❌")
                    st.markdown(
                        f"{icon} **{q.question_id.upper()}** ({q.question_type}, {q.points:.0f}分) — "
                        f"预测得分 **{attempt.estimated_score:.0f}分** · {attempt.error_type}"
                    )
                    if attempt.predicted_answer:
                        st.caption(f"预测作答：{attempt.predicted_answer}")
    ...
    if report.suggestions:
        for sug in report.suggestions:
            priority_color = {"高": "#B91C1C", "中": "#B45309", "低": "#047857"}.get(sug.priority, "#374151")
            st.markdown(
                f"<span style='color:{priority_color};font-weight:700;'>[{sug.priority}优先级]</span> "
                f"**{sug.issue}**",
                unsafe_allow_html=True,
            )
            st.markdown(f"- **建议**：{sug.suggestion}")
            st.markdown(f"- **预期效果**：{sug.expected_impact}")
    else:
        st.info("未生成教学建议。")
    ...
    lines: list[str] = [
        f"# 试卷分析报告",
        f"",
        f"学科: {report.subject} | 年级: {report.grade} | 考试主题: {report.exam_topic}",
        f"",
        f"## 班级总体预测",
        report.class_summary or "（未生成）",
        f"",
        f"## 逐题诊断",
    ]
    for q in report.questions:
        diag = diag_by_qid.get(q.question_id)
        lines.append(f"### {q.question_id.upper()} · {q.question_type} · {q.points:.0f}分")
        lines.append(q.content)
        lines.append(f"知识点: {', '.join(q.knowledge_tags)}")
        if diag:
            lines.append(f"难度: {diag.difficulty_label} | 预测正确率: {int(diag.predicted_class_accuracy*100)}%")
            lines.append(f"常见错误: {diag.common_error_type}")
            lines.append(f"教学诊断: {diag.teaching_note}")
        lines.append("")
    lines.append("## 教学改进建议")
    for sug in report.suggestions:
        lines.append(f"- [{sug.priority}] **{sug.issue}**")
        lines.append(f"  建议: {sug.suggestion}")
        lines.append(f"  预期效果: {sug.expected_impact}")
    md_bytes = "\n".join(lines).encode("utf-8")
    st.download_button(
        label="下载试卷分析报告（Markdown）",
        data=md_bytes,
        file_name=f"exam_report_{report.subject}_{report.exam_topic}.md",
        mime="text/markdown",
        use_container_width=True,
    )